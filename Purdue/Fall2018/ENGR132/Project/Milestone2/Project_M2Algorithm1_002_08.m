function [yL, yH, tS, tau, string] = Project_M2Algorithm1_002_08(file)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ENGR 132 FINAL PROJECT
%
% Function Call
% [yL, yH, tS, tau] = Project_M2Algorithm1_002_08(file)
%
% Input Arguments
% 1. file: a string of a file name that we want to import
%
% Output Arguments
...
% file = 'M2_Data_Calibration_HeatingNoisy.csv';
% The file above is provided for calibration purposes

M2data = csvread(file);
timeCol = M2data(:,1);    % put first column into column vector
                           % containing the time values
tempCol = M2data(:,2);    % put second column into column vector 
                           % containing the temperature values 

%%
%% STEP TWO - Calculating yL and yH
...
%%
%% STEP THREE - Finding the tS value
...
%error of 0.5 degrees from yTau (this 1.5 is arbitrary). And find the
%time values that correspond to those indices.
if y1 > y2     % cooling 
    tauCorrespondIndex = find((yTau - 25)<=tempCol &...
        tempCol <= (yTau + 25));
else           % heating
    tauCorrespondIndex = find((yTau - 3)<=tempCol &...
        tempCol <= (yTau + 3)); 
end
